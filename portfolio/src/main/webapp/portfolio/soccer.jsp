...
<%
    ...
    // 15분이 지난 게시물 삭제
    String deleteSql = "DELETE FROM soccer WHERE CREATED_AT < SYSTIMESTAMP - INTERVAL '15' MINUTE";
...
<body>
    ...
    <section class="title">
        <p>11명으로 구성된 축구 팀을 모집합니다!</p>
        <p>제목에 반드시 지역 + 시합 위치가 포함되어야합니다.</p>
        <p>매치 목록을 확인 후 참여 가능합니다!</p>
        <p>게시물은 15분 뒤 자동 삭제됩니다.</p>
    </section>
    ...
    <section class="main-section">
        ...
        <div class="match-listing">
            <ul>
                <%
                try {
                    String sql = "SELECT * FROM (SELECT MATCH_ID, TITLE, DESCRIPTION, CONTACT, USERNAME, CREATED_AT, ROW_NUMBER() OVER (ORDER BY MATCH_ID) AS rn FROM soccer) WHERE rn BETWEEN ? AND ?";
        ...
        <div class="pagination">
            <%
            // 페이지 수 계산
            String countSql = "SELECT COUNT(*) FROM soccer";
        ...
        %>
            <div class="post-match">
                <h3>매치 게시하기</h3>
                <form action="postSoccerAction.jsp" method="post">
