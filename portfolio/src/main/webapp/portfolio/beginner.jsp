...
<%
    ...
    // 15분이 지난 게시물 삭제
    try {
        String deleteSql = "DELETE FROM BeginnerMatches WHERE CREATED_AT < SYSTIMESTAMP - INTERVAL '15' MINUTE";
        PreparedStatement deleteStmt = jdbc.con.prepareStatement(deleteSql);
        deleteStmt.executeUpdate();
    } catch (SQLException e) {
        out.println("게시물 삭제 오류: " + e.getMessage());
    }
%>
<!DOCTYPE html>
<html lang="ko">
    ...
    <section class="main-section">
        ...
        <div class="match-listing">
            <ul>
                <%
                try {
                    String sql = "SELECT * FROM (SELECT MATCH_ID, TITLE, DESCRIPTION, CONTACT, USERNAME, CREATED_AT, ROW_NUMBER() OVER (ORDER BY MATCH_ID) AS rn FROM BeginnerMatches) WHERE rn BETWEEN ? AND ?";
        ...
        <div class="pagination">
            <%
            // 페이지 수 계산
            String countSql = "SELECT COUNT(*) FROM BeginnerMatches";
        ...
        %>
            <div class="post-match">
                <h3>매치 게시하기</h3>
                <form action="postBeginnerMatchAction.jsp" method="post"> <!-- 변경된 action URL -->
